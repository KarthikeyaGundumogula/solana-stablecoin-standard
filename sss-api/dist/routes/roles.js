...
router.get("/:mint/check", asyncHandler(async (req, res) => {
    ...
    if (hasRole && roleType === RoleType.Minter) {
        try {
            const quotaPda = await getMinterQuotaPda(mint, address);
            ...
            return ok(res, {
                ...
                quota: {
                    ...
                    used: quota.data.quotaUsed?.toString() ?? "0",
                },
            });
        }
        catch {
 ...
 * Body: { address, role }
 */
router.post("/:mint/revoke", asyncHandler(async (req, res) => {
    const { mint } = req.params;
    ...
    const stablecoin = new SolanaStablecoin({ rpc, sendAndConfirmTransaction }, mint);
    const tx = roleType === RoleType.Minter
        ? await stablecoin.updateMinter(authority, authority, address, false, 0n)
        : await stablecoin.updateRoles(authority, authority, address, roleType, false);
    const signature = await sendAndConfirmTransaction(tx, {
        commitment: "confirmed",
    });
    console.log(`[ROLE] Revoked '${role}' from ${address} on ${mint} | ${signature}`);
